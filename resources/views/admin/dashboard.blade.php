    ...
    <div class="container-fluid py-4">
        ...
        <div class="card bg-dark border-danger mb-4 shadow-sm">
            <div class="card-header bg-danger text-white fw-bold">
                <i class="bi bi-exclamation-triangle-fill me-2"></i> تنبيهات المخزون (الكمية نفدت)
            </div>
            <ul class="list-group list-group-flush">
                @php
                    $lowStockNotes = auth()->user()->unreadNotifications->where('type', 'App\Notifications\LowStockNotification');
                @endphp

                @forelse($lowStockNotes as $note)
                    <li class="list-group-item bg-dark border-secondary">
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                <strong class="text-warning">{{ $note->data['title'] }}</strong><br>
                                <small class="text-light">{{ $note->data['message'] }}</small>
                            </div>
                            <small class="text-muted">{{ $note->created_at->diffForHumans() }}</small>
                        </div>
                    </li>
                @empty
                    <li class="list-group-item bg-dark text-muted text-center py-3">
                        <i class="bi bi-check-circle text-success me-1"></i> جميع الكتب متوفرة حالياً.
                    </li>
                @endforelse
            </ul>
            @if($lowStockNotes->isNotEmpty())
                <div class="card-footer bg-transparent text-center">
                    <a href="#" class="btn btn-sm btn-outline-light">تنظيف التنبيهات</a>
                </div>
            @endif
        </div>
        @endrole
                            ...
                            <div class="list-group-item bg-transparent border-secondary px-0">
                                <div class="d-flex w-100 justify-content-between">
                                    ...
                                    <small class="text-warning"
                                        style="font-size: 0.7rem;">{{ $borrow->created_at->diffForHumans() }}</small>
                                </div>
